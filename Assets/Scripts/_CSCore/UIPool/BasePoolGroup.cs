...
public abstract class BasePoolGroup : MonoBehaviour
{
	...
	[SerializeField] protected PoolTypeShow m_TypeShow = PoolTypeShow.NONE;
	...
	protected List<UIPoolObject> listPool = new List<UIPoolObject> ();
	...
	protected bool canShowAnim = true;
	...
	{
		foreach (UIPoolObject poolObj in listPool) {
    ...
    //giangcl
    public List<GameObject> getListGoCellActive()
    {
        List<GameObject> lstRet = new List<GameObject>();
        //
        foreach (UIPoolObject po in listPool)
        {
            if (!po.isAvailable)
            {
                lstRet.Add(po.gameObj);
            }
        }
        //
        return lstRet;
    ...
    protected void getPooledObject (int index)
	{
		...
		//add to list pool
		UIPoolObject po2 = new UIPoolObject ();
		po2.index = index;
